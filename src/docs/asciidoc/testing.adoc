...
* ORM frameworks (such as JPA and Hibernate) that condone `private` or `protected` field
  access as opposed to `public` setter methods for properties in a domain com.spring.entity.
...
* Data access using JDBC or an ORM tool. This can include such things as the correctness
  of SQL statements, Hibernate queries, JPA com.spring.entity mappings, and so forth.
...
that implements data access logic for a `Title` domain com.spring.entity. We want to write
